...
internal sealed class Base
{
	...
	// %LOCALAPPDATA%\Modinator\modinator_log.txt — beside overrides.json /
	// hotkeys.json / prefs.json. It used to sit next to the exe, which is
	// unwritable when the tool is run from Program Files (logging then
	// silently no-op'd into the empty catch below) and hard to describe to
	// someone who downloaded the exe to an arbitrary folder. Debug builds
	// are the diagnostic artifact we ask remote users for, so the log has to
	// land somewhere predictable and always-writable. Surfaced in the
	// Settings diagnostic report and by Settings → OPEN FOLDER.
	public static readonly string LogPath = System.IO.Path.Combine(
		Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
		"Modinator", "modinator_log.txt");

	// ONE SESSION PER FILE. BeginSession truncates at startup, so a user who
	// sends their log sends exactly the run that went wrong — not months of
	// accumulated noise, and with no second file to forget to attach. The cap
	// only guards against a runaway loop filling the disk mid-session.
	private const long LogCapBytes = 5L * 1024 * 1024;
	private static readonly object LogLock = new();
	private static bool _logCapped;

	// Verbose per-tick diagnostics — STILL Debug-only, deliberately. These
	// fire from the Auto-Kill loop and the scan paths; writing them every tick
	// in a shipped build costs real file I/O under a lock for no user benefit.
	[System.Diagnostics.Conditional("DEBUG")]
	public static void Log(string message) => WriteLog(message);

	// Events worth having when something goes wrong: attach failures, scan
	// outcomes, discovery results, crashes. ALWAYS compiled — this is the log
	// users are asked to share, so it has to exist in the Release build.
	public static void LogEvent(string message) => WriteLog(message);

	// Start a fresh session log, discarding the previous run's.
	public static void BeginSession(string header)
	{
		lock (LogLock)
		{
			...
			try
			{
				...
				// A rotated file left by an older build would otherwise sit
				// there forever — and it is exactly the one a confused user
				// attaches instead of the current session.
				string stale = LogPath + ".1";
	...
	{
		if (!Modinator.Prefs.Current.ErrorLogEnabled) return;
		try
		{
			lock (LogLock)
			{
				if (_logCapped) return;
				System.IO.Directory.CreateDirectory(System.IO.Path.GetDirectoryName(LogPath)!);
				if (System.IO.File.Exists(LogPath)
					&& new System.IO.FileInfo(LogPath).Length > LogCapBytes)
				{
					_logCapped = true;
					System.IO.File.AppendAllText(LogPath,
						"[log reached 5 MB — further lines dropped for this session]\n");
					return;
				}
	...
	// PID the Scanner is currently attached to (0 = none / exited). Lets
	// the Auto-Kill handle path target the SAME game instance the user
	// picked in the choose-process dialog instead of "first process found"
	// — with two DunDefGame instances the two paths could otherwise read
	// different processes.
	public static int AttachedPid
	{
		get
		{
			try
			{
				var p = LastProcess;
				if (p != null && Instance.Handle != IntPtr.Zero && !p.HasExited) return p.Id;
			}
			catch { }
			return 0;
		}
	...
	private static Process? GetProcess(bool notify)
	{
		...
		if (processesByName.Length > 1)
		{
			// The chooser is modal too, so a repeating caller must never
			// reach it — same stacking softlock as the error dialog, and
			// notify:false is exactly the "don't put a window in front of
			// the user" contract. Fail quietly instead; once the user
			// attaches through any interactive path the cached handle
			// serves the repeating callers.
	...
	// notify:false attaches silently — for callers on a repeating path (the
	// 50 ms UI timer, a background loop). A modal raised from such a path
	// pumps the message loop, so the caller runs again BEHIND the dialog and
	// raises the next one, stacking dialogs until the app can't be closed.
	// Repeating callers must pass false and report failure in the UI instead.
	public static bool OpenProcess(bool notify = true)
	{
		...
		Process? process = GetProcess(notify);
		if (process == null)
		{
			if (notify)
				OnMessage?.Invoke(
					"Unable to find the 'DunDefGame.exe' process, is the game running?\n\n" +
					"This tool only works with the 32-bit version of Dungeon Defenders. " +
					"The 64-bit version is not supported and will not be detected.",
					"Error");
			return false;
		}
		...
		{
			// Scanner.OpenProcess THROWS when the handle can't be opened —
			// most often UnauthorizedAccessException ("Please run this program
			// as an administrator") because DD1 is elevated and this tool is
			// not. That exception used to escape into async void click
			// handlers and DispatcherTimer ticks, where the shipped x86 build
			// has no global handler at all (App registers one only on ARM64,
			// and even that sets Handled=false) — so a denied attach crashed
			// the app AND buried the one message that tells the user how to
			// fix it. Attach failure is a normal outcome, not a crash: reset
			// the half-set state and report it like any other.
			LastProcess = null;
			MainWindow = IntPtr.Zero;
			LogEvent($"Attach FAILED: {ex.GetType().Name}: {ex.Message}");
			if (notify) OnMessage?.Invoke(ex.Message, "Error");
			return false;
		}
		_lastAttachVerify = DateTime.Now;
		return true;
